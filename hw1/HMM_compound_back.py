...
bitext = [[sentence.strip().split() for sentence in reversed(pair.split(' ||| '))] for pair in codecs.open(opts.bitext,encoding='utf-8')][:opts.num_sents]
bitextCompound = [[sentence.strip().split() for sentence in reversed(pair.split(' ||| '))] for pair in codecs.open(opts.bitextCompound,encoding='utf-8')][:opts.num_sents]
...
def forwardBackward(f, e, trans, align):
    ...
    for j, e_j in enumerate(e): 
        for i, f_i in enumerate(f):
            ...
            if (j==0):
                ...
                beta[(L-j-1, i)] = sum(beta[(L-j, k)]*trans[(e[L-j], f[k+1])]*align[k-i] if k+1<len(f) else beta[(L-j, k)]*align[k-i] for k, f_k in enumerate(f) )
...
def BaumWelch(bitext, NUM_ITER, trans):
    ...
    for it in xrange(NUM_ITER):
        ...
        for (n, (f, e)) in enumerate(bitext):
            xi = {}
            alpha, beta = forwardBackward(f, e, trans, align)    
...
def main():
    ...
    for (f, e), (fc, ec) in zip(bitext, bitextCompound):
        wordMatch = wordMapping(ec)
        alignment = viterbi(f, e, trans, align)
        for i, a in enumerate(alignment):
            if ALIGN_NULL:
                if a < len(fc):
                    sys.stdout.write("%i-%i " % (a, wordMatch[i]))
            else:
                sys.stdout.write("%i-%i " % (a, wordMatch[i]))
        sys.stdout.write("\n")
