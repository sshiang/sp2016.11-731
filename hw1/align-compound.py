...
textFile = "data/parse-de-en-compound.txt"
textFile = "data/parse-de-en-compound-nosemicolon.txt"
#textFile = "data/parse-de-en-compound-small.txt"
	...
	while (square(probs, pre_probs)>=10**-10):
		...
		for i in range(len(examples)):# in examples:
			...
			for word_en in example.eng:
				for word_gex in example.german:
					for word_ge in word_gex:
						if probs == None:
...
for i in range(min(300,len(examples))):
	...
	for index_en in range(len(example.eng)):
		...
		for index_ge in range(len(example.german)):
			...
			for word_ge in example.german[index_ge]:
