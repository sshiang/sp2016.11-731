...
class MyLayer(lasagne.layers.Layer):
...
def build_encoder_network(num_inputs, num_hidden):
    ...
    l_enc = MyLSTMLayer(l_in, num_hidden, mask_input=l_mask, grad_clipping=grad_clip,
                        ...
                        only_return_final=True, name="encoder_net")
...
def build_decoder_network(tvars, enc_net_params, encoded_msg, num_hidden, num_outputs):
    ...
    reset = theano.function([hid_init, input, l_mask.input_var], [posterior, hid_out],
                            givens={encoded_msg: hid_init}, 
                            updates={l_dec.cell_init: cell_init}
                            )

    '''
    predict2 = theano.function([hid_init, input, l_mask.input_var], [posterior, hid_out],
                                givens={encoded_msg: hid_init},
                               )   
    '''
...
def build_encoder_decoder_network():
    ...
    train, predict_and_get_loss, predict, reset, params = build_decoder_network(tvars, enc_net_params, encoded_tvar, n_hidden, VOCAB_TARGET)
...
if __name__ == '__main__':
    ...
    if not args.file:
        print "Training ..."
        batches = get_batches(train_set, batch_size=256, num_epochs=10, randshuf=False)
        ...
        save_model("models/test-model.{}.h5".format(t), params)
    else:
    ...
    for i, (x, x_mask, x_one_hot, y, y_mask, y_one_hot) in enumerate(batches):
        ...
        reset(hid_prev, next_input, mask)
